...
export class ExcelparserService {
  async parseExcelFile(file: Express.Multer.File): Promise<any> {
    ...
    workbook.eachSheet((worksheet, sheetId) => {
      ...
      const subjects: { subject: string, course: string, lek: number, lab: number, sem: number, prakt: number }[] = [];
      ...
      while (true) {
        ...
        if (subject) {
          // Перетворення значення курсу на число (якщо це "1 курс", то курс = "1")
          const courseMatch = course.match(/^(\d+)/); // Шукає лише перше число
          if (courseMatch) {
            course = courseMatch[1]; // Заміна на перше число, якщо воно є
          }
          ...
          let existingSubject = subjects.find(s => s.subject === subject && s.course === course);
      ...
      for (const subjectData of subjects) {
        const totalHours = subjectData.lek + subjectData.lab + subjectData.sem + subjectData.prakt;
